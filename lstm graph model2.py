#epoch vs dev set accuracy model 2
#model 2
#LSTM POS Tagger
import torch
...
class LSTMPOSTagger(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size):
        ...
        self.embedding_transform = nn.Linear(embedding_dim, 256)
        self.lstm = nn.LSTM(256, hidden_dim, batch_first=True, bidirectional=True)
        self.hidden2tag = nn.Linear(hidden_dim * 2, tagset_size)
        self.tanh = nn.Tanh()

    def forward(self, sentence):
        transformed_embedding = self.tanh(self.embedding_transform(sentence))
        lstm_out, _ = self.lstm(transformed_embedding)
...
for epoch in range(10):
    ...
    accuracy_dev, _, _, _, _ = compute_metrics(predictions_test, target_sequences_test, pos_tag_vocab)
    dev_set_accuracies.append(accuracy_dev)
    print(f"Epoch {epoch+1}, Loss: {loss.item()}, Dev Set Accuracy: {accuracy_dev:.4f}")
plt.figure(figsize=(10, 6))
plt.plot(range(1, 11), dev_set_accuracies, marker='o', color='blue', linestyle='-', label='Dev Set Accuracy')
